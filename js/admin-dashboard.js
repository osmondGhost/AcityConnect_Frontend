const hasAdminAccess =
  typeof requireAdmin === "function" ? requireAdmin() : false;
...
async function loadDashboard() {
  ...
  try {
    ...
    if (
      ...
      error.message === "Admin access is not configured"
    ) {
...
async function editItem(itemId) {
  ...
  const status = prompt(
    "New status: available, swapped, or sold (leave blank to keep current):",
  );
...
async function deleteReportedItem(reportId, itemId) {
  ...
  try {
    await apiCall(`/admin/items/${itemId}`, "DELETE");
    await apiCall(`/admin/reports/${reportId}`, "PATCH", {
      status: "resolved",
    });
    await reloadDashboard();
  } catch (error) {
