...
function displayOfferedSkills(skills) {
  ...
  if (skills.length === 0) {
    container.innerHTML =
      '<div class="col-span-full text-center text-gray-500">No skills being offered</div>';
    return;
  }
...
function displayNeededSkills(skills) {
  const container = document
    .getElementById("needed-section")
    .querySelector("div");

  if (skills.length === 0) {
    container.innerHTML =
      '<div class="col-span-full text-center text-gray-500">No skills being requested</div>';
    return;
  }
